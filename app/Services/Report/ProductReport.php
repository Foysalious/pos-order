...
class ProductReport
{
    ...
    public function create()
    {
        $orders = $this->orderRepository->where('partner_id', $this->partner_id)
            ->whereBetween('created_at', [Carbon::parse($this->from)->subHours(6), Carbon::parse($this->to)->subHours(6)])
            ->select('id')
            ->pluck('id');
        ...
        return array_merge(array_values($items_without_id), array_values($product_report));
    }
